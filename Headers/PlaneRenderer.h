...
{
public:
	PlaneRenderer(std::shared_ptr<Graphics> graphics, std::shared_ptr<ShaderFactory> shaderFactory) : BaseRenderer{ graphics }
	{
		...
		shaderData->vertexShaderData = shaderFactory->createVertexShader(L"PlaneVertexShader.cso");
		shaderData->pixelShaderData = shaderFactory->createPixelShader(L"PlanePixelShader.cso");
	}
